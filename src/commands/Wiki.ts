...
export default class WikiCommand extends Command {
    ...
    constructor(context: Command.LoaderContext, options: Command.Options) {
        super(context, {
            ...
            preconditions: [{
                ...
                context: { formats: [{
                    name: 'language',
                    validate: (value: string) => WikiCommand.wikipediaLanguages.includes(value),
        ...
        registry.registerChatInputCommand((builder) =>
            builder
                ...
                .addStringOption((option) =>
                    option
                        .setName('language')
                        .setDescription('Wikipedia language code (the subdomain for the language)')
                        .setRequired(true)
    ...
    public override async autocompleteRun(interaction: AutocompleteInteraction): Promise<void> {
        ...
        const filtered = WikiCommand.wikipediaLanguages
            .filter((lang) => lang.startsWith(focused))
            .slice(0, 25);

        await interaction.respond(
            filtered.map((lang) => ({ name: lang, value: lang }))
        );
    }
